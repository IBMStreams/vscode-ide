...
export default class SplLanguageClient {
  ...
  public static async start(): Promise<any> {
    ...
    let launcher: string;
    let command: string;
    const getCommand = (launcherPath: string): string =>
      os.platform() === 'win32' ? launcherPath : `./${launcherPath}`;
    const cwd = this._context.asAbsolutePath(
      path.join('node_modules', '@ibmstreams', 'spl-lsp', 'bin')
    );
    ...
    if (mode === Settings.SERVER_MODE_VALUE.EMBEDDED) {
      ...
      launcher =
        os.platform() === 'win32'
          ? 'startSplLspServerSocket.bat'
          : 'startSplLspServerSocket';
      command = getCommand(launcher);
      try {
        serverProcess = childProcess.spawn(command, [`port=${port}`], {
          ...processOptions,
          windowsHide: true
        });
    ...
    const toolkitsOption = ToolkitUtils.getLangServerOptionForInitToolkits(
      TOOLKITS_CACHE_DIR,
      Configuration.getSetting(Settings.ENV_TOOLKIT_PATHS)
    );
  ...
  public static createDebugEnv(): any {
    return {
      JAVA_OPTS:
        '-Xdebug -Xrunjdwp:transport=dt_socket,address=8998,server=n,suspend=y',
      ...process.env
    };
  }
