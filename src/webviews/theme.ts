...
export default class ThemeHandler {
    ...
    public initAndHandleThemeChanges(applyStylingCallbackFn: Function): MutationObserver {
        const onThemeChange = (): void => {
            ...
            Object.keys(ThemeToken).forEach((token: string) => {
                this._setCssVariable(ThemeToken[token], this._baseTheme[ThemeToken[token]])
            });

            // Override placeholder text color for dark themes
            if (this._baseTheme === g100) {
                this._setCssVariable(ThemeToken.TEXT_03, this._baseTheme[ThemeToken.TEXT_05]);
            }
