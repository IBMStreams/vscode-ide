...
const JobConfigOverlayContainer = ({ messageHandler, jobName }) => {
  ...
  const importFile = async () => {
    ...
    if (selectedFile) {
      ...
      if (selectedFile.json) {
        ...
        const streamsJobName =
          selectedFile.json?.jobConfigOverlays?.[0]?.jobConfig?.jobName;
        if (streamsJobName) {
          dispatch({
            type: Action.SET_STREAMS_JOB_NAME,
            payload: streamsJobName
          });
        }
      }
    }
  ...
  const exportFile = () => {
    const fileName = `${jobName}-jobConfiguration.json`;
