...
const NewConfigContainer = ({ messageHandler, preselectedBaseImage }) => {
  ...
  useEffect(() => {
    async function getBaseImages() {
      const images = await messageHandler.postMessage({ command: 'get-base-images' });
      if (images) {
        setBaseImages(images);
        if (preselectedBaseImage && !selectedBaseImage) {
          const matchingImage = images.find((image) => image.id === preselectedBaseImage.id) || null;
          if (matchingImage) {
  ...
  if (isLoadingBaseImages) {
    ...
    if (baseImages.length) {
      ...
      selectedDropdownItem = selectedBaseImage ? dropdownItems.find((baseImage) => baseImage.text === selectedBaseImage.id) : null;
    } else {
  ...
  const configurationTypeLabel = (
    <div>
      Selecting <strong>Simple</strong> will automatically create the configuration file and start the edge application image build.
      <br /><br />
      Selecting <strong>Advanced</strong> will allow you to further customize the edge application image build.
    </div>
  );

  const docLink = (
    <Link
      href="https://github.com/IBMStreams/vscode-ide/wiki/Building-edge-applications#build-configuration-properties"
          ...
          <div className="bx--col">
            {
              isLoadingBaseImages
                ? (
                  <>
                    <FormLabel>
                      {getHelpTooltip('Base image', 'The base image used to create the edge application image.')}
                    </FormLabel>
                    <DropdownSkeleton />
                  </>
                )
                : (
                  <ComboBox
                    ...
                    items={dropdownItems}
                    onChange={(e) => { setSelectedBaseImage(e.selectedItem); }}
                    placeholder={dropdownLabel}
                    selectedItem={selectedDropdownItem}
                    titleText={getHelpTooltip('Base image', 'The base image used to create the edge application image.')}
                  />
                )
            }
          </div>
        ...
        <div className="bx--row new-config-container__row">
          <div className="bx--col">
            <TextInput
              type="text"
              id="imageName"
              labelText={getHelpTooltip('Image name', 'The name of the edge application image.')}
              value={imageName}
          ...
          <div className="bx--col">
            <TextInput
              type="text"
              id="imageTag"
              labelText={getHelpTooltip('Image tag', 'The tag of the edge application image.')}
              value={imageTag}
        ...
        <div className="bx--row new-config-container__row">
          <div className="bx--col">
            ...
            <RadioButtonGroup
              ...
              onChange={(value) => { setSelectedNewConfigType(value); }}
              ...
              <div className="bx--row new-config-container__row">
                <div className="bx--col">
                  <div className="new-config-container__instructions">
                    Click on the button below to create a sample image build configuration file. To customize
                    the image, add properties such as <code>pipPackages</code> and <code>rpms</code> to the
                    configuration file. Refer to the {docLink} to learn about the supported configuration properties.
                  </div>
                  <div className="new-config-container__instructions">
                    Once you have completed the configuration, return to this form and click on
                    the <strong>Build image</strong> button to start the build.
                  </div>
                </div>
              </div>
              <div className="bx--row new-config-container__row">
                    ...
                    <div className="new-config-container__file-uploader-item">
                      {
                        selectedNewConfigFile && (
                          <FileUploaderItem
                            ...
                            status="edit"
                            onDelete={() => { setSelectedNewConfigFile(null) }}
                          />
                        )
                      }
                    </div>
